...
## 공개 독자 경험 — 절대 규칙
- 독자가 볼 수 없는 내부 파일 경로, 자동화 도구명, 데이터셋명, 관찰 목록, 프롬프트, 후보·점수·검수 과정은 공개 본문·표·각주·출처에 절대 쓰지 않는다. 예: `_data/...`, `watchlist`, `internal`, `cron`, `자동 실행`.
- 특정 ETF·주식·채권·펀드·코인 목록을 표로 제시하거나 상품별 관찰·추천 섹션을 만들지 않는다. 개별 상품명은 독자가 이해해야 하는 공적 사실을 설명할 때만 최소한으로 언급하고, 매수·매도 판단으로 연결하지 않는다.
- 글은 템플릿을 기계적으로 채우지 않는다. 각 글의 첫 문단은 그날의 구체적 사건·수치·제도에서 출발하고, 본문에는 해당 사안만의 맥락·반론·한계를 넣는다. 상투적인 “체크포인트”, “관찰 포인트”, “~할 수 있습니다”의 반복을 편집 단계에서 줄인다.
- 발행 전 문장을 한 번 더 다듬는다. 불필요한 항목 나열, 중복된 면책 문구, 주어 없는 수동태를 제거하고, 경제 전문 편집자가 독자에게 설명하는 자연스러운 한국어로 고친다.

## 편집 포지셔닝 — Observe · Structure · Sustain
- **Observe(관측)**: 성장·침체, 인플레이션·디플레이션, 금리·환율·정책·국제 흐름을 통해 경제의 계절이 어떻게 변하는지 읽는다.
- **Structure(구성)**: 주식, 채권, 현금성 자산, 금, 원자재, 부동산·리츠 등 자산의 역할과 위험을 원칙 중심으로 설명한다.
...
## CEO/CTO/CFO 관점
- CEO 관점: 올웨더경제 | AllWeatherOS는 장기적으로 “국민이 매일 보는 거시경제·자산배분 운영체제”가 되어야 한다. 주제 폭을 ETF에 가두지 말고 경제·사회·문화·국제질서를 관통하는 브랜드로 키운다.
...
6. 글 구조는 반드시 포함한다.
   ...
   - 오늘의 시장 한눈에 보기 표
   - 오늘의 핵심 경제 뉴스
   - 경제의 계절: 성장·침체, 인플레이션·디플레이션, 금리·환율·정책 가운데 무엇이 변하고 있는지 사실과 해석을 구분한다.
   - 오늘의 거시경제 생활 연결: 뉴스가 물가·일자리·소비·세금·연금·자산가격 중 무엇을 건드리는지 설명한다.
   - 자산별 역할과 위험: 주식·채권·현금성 자산·금·원자재·부동산·리츠 가운데 관련 자산의 역할과 위험을 함께 설명한다.
   - 절세계좌와 장기 투자자 체크포인트
   - 투자 유의사항과 결론 한 문장
   - FAQ 5개 이상
   - FAQ 뒤에는 사람이 읽는 흐름을 위해 반드시 아래 순서로 배치한다:
     1. 전문용어 쉽게 풀어쓰기: 일반인이 낯설 수 있는 금융·거시경제·ETF 용어 5~8개를 쉬운 한국어로 설명한다. 예: `장기금리 = 오래 돈을 빌릴 때 적용되는 이자율`, `괴리율 = ETF 가격과 실제 자산가치가 얼마나 벌어졌는지 보는 차이`.
     2. 시장/핵심 데이터 정리
     3. 출처와 확인 근거
     4. 이 글과 함께 읽어보세요(사이트 layout의 related-reading 블록이 자동으로 붙는 경우, 본문은 출처와 참고 근거에서 끝내고 JSON-LD만 둔다)
...
10. 검증:
   ...
   - 공개 독자 경험 검사: 새 글에 `_data/`, `watchlist`, `internal`, `cron`, `자동 실행`, `AllWeatherOS 관심 ETF`가 없는지 확인한다. 특정 금융상품 목록 표·종목코드·목표가·목표 수익률·매수 시점·비중 지시가 있으면 발행을 중단하고 수정한다.
   - 문체 검사: 템플릿 문구·면책 문구·“체크포인트/관찰 포인트”의 반복을 줄이고, 해당 사안의 구체적 맥락과 한계를 담았는지 확인한다.
   - `git diff --check`
   - `git status --short`
11. 커밋/푸시:
   - `git add .`
...
14. 텔레그램 최종 보고는 한국어로 간결하게 한다. 반드시 완료 여부와 **확인 가능한 GitHub Pages 발행 URL**을 포함한다. 내부 후보군·점수표·선정 과정은 어떤 경우에도 보고하지 않는다. 형식 예시: `✅ 발행 완료: [제목]\nURL: https://...\nLIVE_URL_VERIFIED=yes · EMAIL_STATUS=...`. 빌드·배포·라이브 URL·이미지·메타·이메일 검증 중 하나라도 실패하여 사용자 조치가 필요한 경우에는 원인·영향·검증 URL/로그 요약을 함께 보고한다.

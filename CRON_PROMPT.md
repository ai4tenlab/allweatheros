...
- **Observe(관측)**: 성장·침체, 인플레이션·디플레이션, 금리·환율·정책·국제 흐름을 통해 경제 상황이 어떻게 변하는지 읽는다.
...
6. 글 구조는 반드시 포함한다.
   ...
   - 3줄 요약
   - 한 문장 답변
   - 본문: 오늘의 시장 한눈에 보기 표, 오늘의 핵심 경제 뉴스, 오늘의 경제 흐름, 거시경제 생활 연결, 자산별 역할과 위험, 절세계좌와 장기 투자자 체크포인트를 자연스럽게 묶는다. 처음 나오는 어려운 핵심 용어는 본문에서 한 문장으로 즉시 풀어준다.
   - 자연스러운 한국어: `경제의 계절`, `방어력을 시험하는 구간`처럼 영어식 은유를 직역한 표현은 쓰지 않는다. 소제목은 멋있는 말보다 독자가 실제로 묻는 질문에 가깝게 쓴다.
   - 투자 유의사항과 결론 한 문장
   - FAQ 5개 이상
   - 전문용어 쉽게 풀어쓰기: FAQ 뒤, 출처 앞에 둔다. 일반인이 낯설 수 있는 금융·거시경제·ETF 용어 5~8개를 쉬운 한국어로 설명한다. 예: `장기금리 = 오래 돈을 빌릴 때 적용되는 이자율`, `괴리율 = ETF 가격과 실제 자산가치가 얼마나 벌어졌는지 보는 차이`.
   - 출처와 확인 근거
